...
# Three heuristic functions "runaway()", "runtowards()", "custom_score()"

def runaway(game, player):
    ...
    if self_location == None:
        return 0.

    return float(abs(sum(opp_location) - sum(self_location)))


def runtowards(game, player):
...
def custom_score(game, player):
    ...
    board_factor = 1
    # Try to avoid being cornered towards end game
    ...
    self_in_corner = [move for move in self_moves if move in corners]
    ...
    return float(len(self_moves) - (board_factor * len(self_in_corner))
                 - len(opp_moves) + (board_factor * len(opp_in_corner)))
